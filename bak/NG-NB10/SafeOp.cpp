...
namespace SafeOp {
    ...
    static std::vector<bool> mopF, flashF, meteorF,
        fireF;
    static std::vector<int> lastFire, lastMeteor,
        lastFlash;
    ...
    void move(int num, const Point& p,
        ...
        if(num >= humanSiz()) {
            error(callPos) << "move:invaild num"
                           << std::endl;
            return;
        }
        Point cp = getMyUnit(num).position;
        info() << "move " << num << " from [" << cp.x
               << " " << cp.y << "] to [" << p.x << " "
               << p.y << "] dist=" << dist(cp, p)
               << std::endl;
        if(!checkPoint(p) || MapHelper::isWall(p) ||
           dist(cp, p) > CONST::flash_distance) {
            error(callPos) << "move:invalid pos "
                           << std::endl;
            return;
        }
        if(mopF[num]) {
    ...
    void fire(int num, const Point& p,
              const SourceLocation& callPos) {
        if(num >= humanSiz()) {
            error(callPos) << "fire:invaild num"
                           << std::endl;
            return;
        }
        if(!checkPoint(p) ||
           !canFire(getMyUnit(num))) {
            error(callPos) << "fire:can not fire ["
                           << p.x << "," << p.y << "]"
                           << std::endl;
            return;
        }
        if(fireF[num]) {
            ...
            return;
        } else if(lastFire[num] +
                      CONST::human_fireball_interval >
                  getLogic().frame) {
            error(callPos)
                << "no fire delay but can fire:id="
                << num << ",last=" << lastFire[num]
                << std::endl;
        } else {
            fireF[num] = true;
    ...
    void flash(int num,
               const SourceLocation& callPos) {
        if(num >= humanSiz()) {
            error(callPos) << "flash:invaild num"
                           << std::endl;
            return;
        }
        if(!canFlash(getMyUnit(num))) {
            error(callPos) << "flash:can not flash"
                           << std::endl;
            return;
        }
        if(flashF[num]) {
            ...
            return;
        } else if(lastFlash[num] +
                      CONST::human_flash_interval >
                  getLogic().frame) {
            error(callPos)
                << "no flash delay but can flash:id="
                << num << ",last=" << lastFlash[num]
                << std::endl;
        } else {
            flashF[num] = true;
    ...
    void meteor(int num, const Point& p,
                const SourceLocation& callPos) {
        if(num >= humanSiz()) {
            error(callPos) << "meteor:invaild num"
                           << std::endl;
            return;
        }
        if(!checkPoint(p) ||
           !canMeteor(getMyUnit(num))) {
            error(callPos) << "meteor:can not meteor ["
                           << p.x << "," << p.y << "]"
                           << std::endl;
            return;
        }
        ...
        else if(lastMeteor[num] +
                    CONST::human_meteor_interval >
                getLogic().frame) {
            error(callPos)
                << "no meteor delay but can meteor:id="
                << num << ",last=" << lastMeteor[num]
                << std::endl;
        } else {
            meteorF[num] = true;
    ...
    void begin() {
        ...
        if(static_cast<int>(lastMove.size()) == siz) {
            for(int i = 0; i < siz; ++i) {
                if(getMyUnit(i).hp <= 0)
                    continue;
                if(dist(lastMove[i], Point(-1, -1)) <
                   Math::bias)
                    continue;
                Point exc = lastMove[i],
                      cur = getMyUnit(i).position;
                if(dist(cur, exc) > minTrans * 0.1)
                    ERROR << "invalid move " << i
                          << " exc:[" << exc.x << ","
                          << exc.y << "] cur:["
                          << cur.x << "," << cur.y
                          << "]" << std::endl;
