...
Point maxFire(const Point& base) {
    ...
    for(int i = 0; i < sampleCnt; ++i) {
        ...
        Point dir = Point(cos(ang), sin(ang)), off = dir * CONST::room_size,
...
Point sampleDst(int id, const Point& base, FT radius) {
    ...
    if(dist(odst, base) < Math::bias &&
       dist(pos, getMyUnitPos(id)) > minTrans * 1.1 && dist(pos, base) < radius)
        return pos;
    odst = base;
    pos = circleSampler(base, radius).request(!isWallFunc()).sample(512);
    if(pos != Math::invalidPos)
        return pos;
    return base;
}
Point path(int id, const Point& dst, FT radius) {
...
void doMove(int num, const Point& dst, const SourceLocation& callPos) {
    if(canFlash(getMyUnit(num)) &&
       dist(dst, getMyUnitPos(num)) > CONST::human_velocity - Math::bias)
...
bool evalFireBall(const Point& base, const Fireball& x, FT& d, FT& dis,
                  Point& dir, bool def) {
...
FT evalSafeFacFlash(const Point& base) {
    FT mint = 2.0 * CONST::frames_per_second;
...
std::vector<std::pair<Point, Point>> preSafeFac(const Point& base) {
    ...
    for(const Fireball& x : getLogic().fireballs) {
        if(!isAnti(x.from_number))
            continue;
        const int predict = 2 * CONST::frames_per_second;
...
FT evalSafeFacMove(const Point& base, const Point& mdir,
    ...
    for(std::pair<Point, Point> fb : fbs) {
        ...
        for(int i = 1; i <= predict; ++i) {
            ...
            if(dist(fb.first, cp) < CONST::splash_radius) {
                ht = i;
                break;
            }
            if(isWall(fb.first) || isWall(cp))
                break;
        }
        hitTime.push_back(ht);
    }
    std::sort(hitTime.begin(), hitTime.end());
    while(hitTime.size() < 3)
        hitTime.push_back(predict + 1);
    while(hitTime.size() > 3)
        hitTime.pop_back();
    FT sum = 0.0;
    for(int ht : hitTime)
        sum = sum * (predict + 2) + ht;
    return sum;
}
