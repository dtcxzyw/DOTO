...
void updateBonus() {
    ...
    for(size_t i = 0; i < bcnt; ++i)
        if(state[i] && !getLogic().bonus[i]) {
            info() << "bonus " << i << " disappeared."
                   << std::endl;
            lastBonus[i] = getLogic().frame;
        }
    state = getLogic().bonus;
}
void calcBonus(FT mmdis, FT emdis, int bid, int tid,
               int eid, Task& mop) {
    bool willGen = getLogic().frame - lastBonus[bid] >
        CONST::bonus_time_low -
            1.5 * mmdis / CONST::human_velocity;
    if(willGen) {
        mop.radius = -1.0;
        return;
    }
    bool relax = emdis < 0.1 ||
        std::max(mmdis, emdis) > CONST::bonus_radius;
    mop.radius = relax ?
        static_cast<FT>(CONST::bonus_radius) :
        -1.0;
    if(!relax && eid != -1) {
        Point op = getMyUnit(tid).position;
        Point ep = getUnit(eid).position;
        if(emdis < 0.1) {
            bool dir =
                dist(op, ep) < CONST::splash_radius;
            auto evalClose = [&](const Point& cp) {
                return fabs(dist(cp, ep) -
                            CONST::splash_radius);
            };
            for(int i = 0; i < 1024; ++i) {
                Point cp = Math::sampleCircle(
                    op, 0.0, CONST::human_velocity);
                if(dir ^ (dist(cp, ep) <
                          CONST::splash_radius)) {
                    if(evalClose(mop.dst) >
                       evalClose(cp))
                        mop.dst = cp;
                }
            }
...
std::vector<Task> calcTask(int& mid) {
    ...
    // bonus
    {
        ...
        auto toPos = [](int id) {
            return getMap().bonus_places[id];
        };
        std::sort(
            bp.begin(), bp.end(), [&](int a, int b) {
        ...
        for(size_t i = 0; i < bp.size(); ++i) {
            ...
            calcBonus(mdis, emdis, bp[i], tid, eid,
                      mop);
            mop.taskName = "bonus";
            mops.push_back(mop);
        }
