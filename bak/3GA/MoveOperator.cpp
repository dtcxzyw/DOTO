...
Point closestEnemy(const Point& p) {
    int obj = makeAttackBaseSampler()
                  ...
                  .sample();
    return obj == -1 ? p : getUnit(obj).position;
}
...
Point doBonusImpl(int id, int bid, int obj) {
    ...
    Point mp =
        stableSampler(cp, canFlash(hu) ? CONST::flash_distance :
            ...
            .apply(fleeFireball(id))
            .apply(flashBack(id))
            .apply(limit < 0.0 ? PointApplyor{ [=](PointSampler& sampler) {
                sampler.estimate(distance(op));
            } } :
                                 PointApplyor{ [=](PointSampler& sampler) {
                                     sampler.optional(distance(bp) < limit)
                                         .estimate(minv(distance(op)));
                                 } })
            .sample(1024, randomMove(id));
    doMove(id, mp, SRCLOC);
    return mp;
...
Point doBonus(int id, int bid) {
    ...
    int obj = makeAttackBaseSampler()
                  ...
                  .sample();
    if(obj != -1 &&
       std::max(dist(cp, bp), minDist(bp, false)) < CONST::bonus_radius &&
       !inMeteor(cp))
...
std::map<int, Point> doTrans(const std::vector<int>& ids, int mid) {
    ...
    if(!close(ids) && (isLocked(mid, getMyTarget()) ||
