...
using PredFunc = std::function<Bool(const Input&)>;
// Maximum
template <typename Input>
using EvalFunc = std::function<FloatingPoint(const Input&)>;

using PointPredFunc = PredFunc<Point>;
using PointEvalFunc = EvalFunc<Point>;
class PointSampler;
using PointApplyor = std::function<void(PointSampler&)>;

class PointSampler final {
private:
    std::function<Point()> mGen;
    std::vector<PointPredFunc> mReqs;
    std::vector<PointEvalFunc> mEvals;

public:
    explicit PointSampler(const std::function<Point()>& src);
...
using Int2PointFunc = std::function<Point(int)>;
class DiscreteSampler;
using DiscreteApplyor = std::function<void(DiscreteSampler&)>;
...
EvalFunc<Input> prec(const EvalFunc<Input>& func, FT k) {
    return [func, k](const Input& in) {
        return round(func(in).get() / k);
    };
}

template <typename Input>
EvalFunc<Input> offset(const EvalFunc<Input>& func, FT rad) {
    return [func, rad](const Input& in) {
        return func(in).get() + Math::uniform(-rad, rad);
    };
}
...
PredFunc<Input> operator>(const EvalFunc<Input>& fa,
    ...
    return
        [=](const Input& in) { return fa(in).get() > fb(in).get(); };
}
template <typename Input>
PredFunc<Input> operator<(const EvalFunc<Input>& fa,
                          const EvalFunc<Input>& fb) {
...
PredFunc<Input> operator|(const PredFunc<Input>& fa,
    ...
    return [=](const Input& in) {
        return fa(in).test() || fb(in).test();
    };
}

template <typename Input>
PredFunc<Input> operator|(const PredFunc<Input>& fa, bool val) {
    return val ? PredFunc<Input>([](const Input&) { return true; }) :
                 fa;
}
