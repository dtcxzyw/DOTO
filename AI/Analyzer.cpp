...
bool isLocked(int id, const Point& dst) {
    ...
    const size_t sampleCnt = CONST::frames_per_second / 2;
    info.his.push_back(cp);
    while(info.his.size() > sampleCnt)
        info.his.pop_front();
    static std::map<int, int> lastLocked;
    if(info.his.size() == sampleCnt) {
        bool locked = dist(info.his.front(), cp) < 3.0 * CONST::human_velocity;
        if(locked)
            lastLocked[id] = getLogic().frame;
    }
    return lastLocked.count(id) &&
        getLogic().frame - lastLocked[id] < CONST::frames_per_second * 4;
}
