...
private:
    ...
    std::vector<Point> mHots;
    std::vector<PointPredFunc> mHotReqs;

public:
    explicit PointSampler(const PointGenerator& src);
    PointSampler& interest(const std::vector<Point>& hots);
    PointSampler& hotRequest(const PointPredFunc& req);
