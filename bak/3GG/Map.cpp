...
Point sampleDst(int id, const Point& base, FT minRad, FT maxRad) {
    ...
    pos = PointSampler([=](int, int) {
              ...
              .hotRequest(distance(base) > minRad)
              .hotRequest(distance(base) < maxRad)
              .request(!isWallFunc())
              .sample(512);
    ...
    if(noWall(hu.position, cdst))
        ...
        info() << "astar "
               << std::chrono::duration_cast<std::chrono::microseconds>(dur)
                      .count()
               << "us" << std::endl;
        if(res == Math::invalidPos) {
            ERROR << "astar error" << std::endl;
            return dst;
        }
        return res;
    }
...
void doMove(int num, const Point& dst, const SourceLocation& callPos) {
    info() << "call from " << callPos.file << ":" << callPos.line << std::endl;
