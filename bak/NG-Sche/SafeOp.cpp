...
namespace SafeOp {
    ...
    void profile() {
        ...
        FT totFlash =
            frame / CONST::human_flash_interval;
        FT totMove = (frame - totFlash) *
                CONST::human_velocity +
            totFlash * CONST::flash_distance;
        debug() << "move ratio:"
                << 20.0 * sumMove / totMove << "%"
                << std::endl;
        FT totFire =
            frame / CONST::human_fireball_interval;
        debug() << "fire ratio:"
                << 20.0 * fireCnt / totFire << "%"
                << std::endl;
        FT totMeteor =
            frame / CONST::human_meteor_interval;
        debug() << "meteor ratio:"
                << 20.0 * meteorCnt / totMeteor << "%"
                << std::endl;
    }
}
