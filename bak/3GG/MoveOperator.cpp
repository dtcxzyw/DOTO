...
PointApplyor fleeFireballFlash(const Point& mp) {
    return [=](PointSampler& sampler) {
        sampler.estimate([=](const Point& p) { return evalSafeFacFlash(p); })
            .optional(!inFireFunc())
            .optional(noWallFunc(mp));
    };
}
PointApplyor fleeFireball(int id) {
    const Human& hu = getMyUnit(id);
    return canFlash(hu) ? fleeFireballFlash(hu.position) :
...
Point doBonus(int id, int bid) {
    ...
    if(obj != -1 &&
       std::max(dist(cp, bp), dist(bp, getUnit(obj).position)) <
           CONST::bonus_radius &&
       !inMeteor(cp) &&
       (getLogic().scores[getMyFac()] > getLogic().scores[getFac(obj)] ||
        dist(bp, getUnit(obj).position) > minTrans))
...
std::map<int, Point> doTrans(const std::vector<int>& ids, int mid) {
    ...
    Point dst = help ? mps[mid] : closestEnemy(mps[mid]);
    for(int id : ids)
        if(id != mid)
            mps[id] = defaultMoveTo(id, dst, closeDis, 20.0);
    return mps;
