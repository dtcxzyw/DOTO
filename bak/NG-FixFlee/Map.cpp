...
Point tryMove(int num, Point dir,
    ...
    bool out = false;
    for(Point old : cnxt)
        if(dist(hu.position, old) < keep) {
            out = true;
            break;
        }
    if(!out && length(dir) > Math::bias) {
...
const FT maxFireDis = 3.0 * CONST::frames_per_second *
...
FT evalSafeFac(const Point& base, const Point& mdir) {
    ...
    int tot = 0;
    const int end =
        maxFireDis / (CONST::fireball_velocity *
                      CONST::human_fireball_interval);
    for(size_t i = 0; i < end && i < ts.size(); ++i)
        sum += ts[i], ++tot;
    return tot ? sum / tot : 1e20;
}
...
Point calcMove(const std::vector<Point>& cnxt, int id,
    ...
    debug() << "Safe fac " << id << "=" << sfac
            << std::endl;
    if(sfac > CONST::frames_per_second) {
        ...
        if(MapHelper::noWall(hu.position, cdst))
            ...
            return tryMove(id, pos - hu.position,
                           cnxt);
        }
    } else {
        ...
        for(int i = 0; i < sampleCnt; ++i) {
            ...
            Point nxt =
                pos + cd * CONST::human_velocity;
            if(MapHelper::isWall(nxt))
                continue;
            FT cval = evalSafeFac(pos, cd);
            if(cval > mval)
                mval = cval, dir = cd;
        }
        debug() << "max safe fac = " << mval
                << std::endl;
